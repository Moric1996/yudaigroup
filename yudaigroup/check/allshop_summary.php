...
foreach($ybase->section_list as $key => $val){
	...
	if($num){
		list($q_ckaction_id,$q_ckset_id,$q_employee_id,$q_action_date,$q_pass_date) = pg_fetch_array($result,0);
		...
		foreach($category_list as $key2 => $val2){
			$sumval[$key][$key2] = 0;
			$sumken = 0;
			if(!isset($subject_list_cate_arr[$key2])){
				$subject_list_cate_arr[$key2] = array();
			}
...
$ybase->title = "店舗チェック-サマリーページ";

$ybase->HTMLheader();
$ybase->ST_PRI .= $ybase->header_pri("サマリーページ");
$ybase->ST_PRI .= $check->check_menu("2");
...
<div class="container">
<h5 style="text-align:center;">サマリーページ</h5>
...
if($pass_date_arr[$key] > 14){
	$pass_color = " style=\"color:#ff5555;\"";
}else{
	$pass_color = "";
}
