...
if(preg_match("/^[0-9]+$/",$sel_slip_id)){
		$add_sql .= " and a.slip_id = $sel_slip_id";
}
if(preg_match("/^[0-9]+$/",$sel_money)){
		$add_sql .= " and cast(a.money as text) ~'$sel_money'";
}
if($sel_slip_type != 3){
	$sel_credit_id = "";
}
if(preg_match("/^[0-9]+$/",$sel_credit_id)){
		$add_sql .= " and a.credit_id = $sel_credit_id";
}
...
$param = "sel_status=$sel_status&sel_slip_type=$sel_slip_type&sel_action_date_st=$sel_action_date_st&sel_action_date_ed=$sel_action_date_ed&sel_month_st=$sel_month_st&sel_month_ed=$sel_month_ed&sel_section_id=$sel_section_id&sel_charge_emp=$sel_charge_emp&sel_supplier=$sel_supplier&sel_account=$sel_account&sel_money=$sel_money&sel_credit_id=$sel_credit_id".$accept_param;
//////////////////////////////////////////
$no_release_flag = $slip->no_release_check($ybase->my_employee_id);

if($no_release_flag){
	$release_sql = " and ((a.charge_emp = $ybase->my_employee_id) or (a.no_release = '0'))";
}else{
	$release_sql = "";
}
...
$(function(){
	$('#Filter_Form1 select,#Filter_Form1 input').change(function(){
...
$(function(){
	$("#alldl").click(function(){
		$('#downloadbody1 :checkbox:checked').each(function() {
			var slipatno_val = $(this).attr('slipatno');
//			$('#dl' + slipatno_val).trigger("click");
			let url = $('#dl' + slipatno_val).attr('href');
			dlFile(url);
		});
	});
	function dlFile(url) {
...
$(function(){
	$('#allcheck').click(function(){
		$("[id^=downloadck]").prop("checked", $(this).prop("checked"));
	});
});
...
<div class="text-left small">
<form action="slip_list.php" method="post" id="Filter_Form1">
...
<div class="form-group col-8 col-sm-6">
<label>【　対象月　】</label>
<nobr><input type="month" name="sel_month_st" value="$sel_month_st">～<input type="month" name="sel_month_ed" value="$sel_month_ed"></nobr>
</div>
</div>
...
<select name="sel_supplier" id="sel_supplier">
<option value="">全て</option>
HTML;
...
<div class="form-row ">
<div class="form-group col-8 col-sm-6">
<label><nobr>【　金　額　】</nobr></label>
<input type="number" name="sel_money" value="$sel_money" id="sel_money">
</div>
<div class="form-group col-8 col-sm-6">
<label><nobr>【利用カード】</nobr></label>
<select name="sel_credit_id"{$credit_disable}>
<option value="">全て</option>
HTML;

foreach($creditcart_list as $key => $val){
	if($sel_credit_id == $key){
		$addselect = " selected";
	}else{
		$addselect = "";
	}
...
<p></p>
<table class="table table-bordered table-hover table-sm" style="font-size:78%;padding:0px;margin:0px;" id="sliptable1">
  <thead>
...
<span class="mx-1">メモ</span>
      ...
      <th scope="col">添付<br><div class="text-left"><input type="checkbox" id="allcheck" value="1">全てチェック</div></th>
    </tr>
  </thead>
...
HTML;
for($i=$st;$i<$end;$i++){
	list($q_slip_id,$q_slip_type,$q_month,$q_action_date,$q_company_id,$q_section_id,$q_money,$q_supplier,$q_supplier_other,$q_account,$q_fee_st,$q_contents,$q_attach,$q_charge_emp,$q_last_accept_list_id,$q_memo,$q_pay_date,$q_add_date,$q_up_date,$q_status,$q_pay_limit,$q_no_release,$q_credit_id) = pg_fetch_array($result,$i);
...
$param00 = "sel_status=$sel_status&sel_slip_type=$sel_slip_type&sel_action_date_st=$sel_action_date_st&sel_action_date_ed=$sel_action_date_ed&sel_month_st=$sel_month_st&sel_month_ed=$sel_month_ed&sel_section_id=$sel_section_id&sel_charge_emp=$sel_charge_emp&sel_supplier=$sel_supplier&sel_account=$sel_account&sel_slip_id=$sel_slip_id";
...
<br><br>{$d_no_release}<br><br><br>
<a href="./update_slip_fm.php?t_slip_id=$q_slip_id&$param00" type="button" delhref="./slip_del.php?slip_id=$q_slip_id" class="btn btn-secondary btn-sm" style="font-size:80%;">変更</a>
<br><br>
...
<div class="form-row ">
<div class="form-group col-sm-6">
<b style="font-size:70%;">【利用カード】</b>{$creditcart_list[$q_credit_id]}
</div>
</div>
...
<div class="text-center">
<a href="./newslip_fm.php?cp_slip_id=$q_slip_id" target="_blank" type="button" class="btn btn-secondary btn-sm" style="font-size:80%;">コピーして新規作成</a>
</div>
</td>
<td align="right">
<b>{$d_money}</b>
...
foreach($q_attach_arr as $key => $val){
	...
	$ext = strtolower($ext);
	$slipatno = $q_slip_id.sprintf("%02d",$key);
////////////////////
	if($ext == 'pdf'){
		$pdffile = urlencode("https://yournet-jp.com/yudaigroup/slip/view.php?slip_id=$q_slip_id&attach_no=$key");
		$d_attach .= "<iframe width=\"300\" height=\"300\" src=\"../inc/pdfjs/web/viewer3.html?file=$pdffile\"></iframe><br><input type=\"checkbox\" name=\"downloadck[{$slipatno}]\" value=\"1\" id=\"downloadck{$slipatno}\" slipatno=\"{$slipatno}\" class=\"mr-5\" style=\"transform: scale(1.5)\"><a href=\"./view.php?slip_id=$q_slip_id&attach_no=$key\" target=\"_blank\" type=\"button\" class=\"btn btn-secondary btn-sm\">拡大</a>　<a href=\"./dl.php?slip_id=$q_slip_id&attach_no=$key\" type=\"button\" class=\"btn btn-secondary btn-sm\" id=\"dl{$slipatno}\">ダウンロード</a><br>";
	}else{
		$d_attach .= "<iframe width=\"300\" height=\"300\" src=\"./slip_iframe.php?slip_id=$q_slip_id&attach_no=$key&ext=$ext\"></iframe><br><input type=\"checkbox\" name=\"downloadck[{$slipatno}]\" value=\"1\" id=\"downloadck{$slipatno}\" slipatno=\"{$slipatno}\" class=\"mr-5\" style=\"transform: scale(1.5)\"><a href=\"./view.php?slip_id=$q_slip_id&attach_no=$key\" target=\"_blank\" type=\"button\" class=\"btn btn-secondary btn-sm\">拡大</a>　<a href=\"./dl.php?slip_id=$q_slip_id&attach_no=$key\" type=\"button\" class=\"btn btn-secondary btn-sm\" id=\"dl{$slipatno}\">ダウンロード</a><br>";
	}
}
	$sql3 = "select dl_log_id,to_char(add_date,'YYYY/MM/DD HH24:MI:SS') from download_log where slip_id = $q_slip_id and attach_no = $key and employee_id = {$ybase->my_employee_id} and status = '1' order by dl_log_id";
	$result3 = $ybase->sql($conn,$sql3);
	$num3 = pg_num_rows($result3);
	$dl_list = "";
	for($iii=0;$iii<$num3;$iii++){
	list($dl_log_id,$dl_add_date) = pg_fetch_array($result3,$iii);
		$dl_list .= "<small class=\"text-center\">$dl_add_date DL</small><br>\n";
	}
...
<td></td>
<td></td>
<td align="center"><a href="#" type="button" class="btn btn-secondary btn-sm" id="alldl">チェックしたものをダウンロード</a>
<button class="btn btn-sm btn-outline-dark mt-3" type="submit" id="dlcsvbtn">チェックした証憑用CSVダウンロード</button>
</td>
</tr>
</form>
 </tbody>
