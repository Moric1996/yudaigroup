...
if(!preg_match("/^[0-9]+$/",$entry_id)){
	$sql = "select NEXTVAL('answerdata_entry_id_seq')";
	$result0 = $base->sql($conn,$sql);
	$entry_id = @pg_fetch_result($result0,0,0);
	$sql = "insert into answerdata (entry_id,add_date) values($entry_id,'now')";
...
$sql = "update answerdata set number = $MS_NUMBER ,employee_num = '$employee_num',belong = $belong,name = '$namae',kana = '$namaekana',superior = '$superior',add_date = 'now',comp_flag = '0',ua = '$agent',ip = '$base->remote_addr' where entry_id = $entry_id";
$result = $base->sql($conn,"$sql");

header("Location: http://yournet-jp.com/ms/form.php?entry_id=$entry_id&main_item_no=1");
exit;
?>