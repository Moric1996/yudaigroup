...
<form action="zkkak_view.php" method="post" id="form1">
...
$sql = "select sale_date,price_range,max(price_range_start),max(price_range_end),sum(totalsale1_notax_price),sum(totalsale1_notax_score),sum(num_custom),sum(num_pair),sum(totalsale3_notax_price) from pos_zkkak where pos_shopno = $pos_shopno and sale_date {$date_sql} and status = '1' group by sale_date,price_range order by sale_date,price_range";
...
for($i=0;$i<$num;$i++){
	list($q_sale_date,$q_price_range,$q_start_time,$q_end_time,$q_totalsale1_notax_price,$q_totalsale1_notax_score,$q_num_custom,$q_num_pair,$q_totalsale3_notax_price) = pg_fetch_array($result,$i);
	$check_start_time = $q_start_time;
	if($q_price_range == 999999){
		$d_price_range = number_format($q_start_time)."円以上";
	}else{
		$d_price_range = number_format($q_start_time)."～".number_format($q_end_time)."円";
	}
...
$ybase->ST_PRI .= <<<HTML
<td align="center">$d_price_range</td>
...
HTML;
$graphtxt .= "data.addRow(['{$d_day}{$d_price_range}',$q_totalsale1_notax_price]);\n";
