...
<form action="zfloor_view.php" method="post" id="form1">
...
$sql = "select sale_date,floor_no,max(floor_name),sum(totalsale1_notax_price),sum(totalsale1_notax_score),sum(num_custom),sum(num_pair),sum(totalsale3_notax_price) from pos_zfloor where pos_shopno = $pos_shopno and sale_date {$date_sql} and status = '1' group by sale_date,floor_no order by sale_date,floor_no";
...
for($i=0;$i<$num;$i++){
	list($q_sale_date,$q_floor_no,$q_floor_name,$q_totalsale1_notax_price,$q_totalsale1_notax_score,$q_num_custom,$q_num_pair,$q_totalsale3_notax_price) = pg_fetch_array($result,$i);
	$q_floor_name = trim($q_floor_name);
...
$ybase->ST_PRI .= <<<HTML
<td align="center">$q_floor_name</td>
...
HTML;
$graphtxt .= "data.addRow(['{$d_day}{$q_floor_name}',$q_totalsale1_notax_price]);\n";
