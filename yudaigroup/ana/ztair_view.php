...
<form action="ztair_view.php" method="post" id="form1">
...
$sql = "select sale_date,detent_time,max(detent_start),max(detent_end),sum(totalsale1_notax_price),sum(totalsale1_notax_score),sum(num_custom),sum(num_pair),sum(totalsale3_notax_price) from pos_ztair where pos_shopno = $pos_shopno and sale_date {$date_sql} and status = '1' group by sale_date,detent_time order by sale_date,detent_time";
...
for($i=0;$i<$num;$i++){
	list($q_sale_date,$q_detent_time,$q_start_time,$q_end_time,$q_totalsale1_notax_price,$q_totalsale1_notax_score,$q_num_custom,$q_num_pair,$q_totalsale3_notax_price) = pg_fetch_array($result,$i);
	$check_start_time = $q_start_time;
	$c_detent_time = sprintf('%04d',$q_detent_time);
	if($c_detent_time == 9999){
		$d_detent_time = substr($pre_detent_time,0,2)."時間".substr($pre_detent_time,2,2)."分以上";
	}else{
		$d_detent_time = substr($c_detent_time,0,2)."時間".substr($c_detent_time,2,2)."分まで";
	}
	$pre_detent_time = $c_detent_time;
...
$ybase->ST_PRI .= <<<HTML
<td align="center">$d_detent_time</td>
...
HTML;
$graphtxt .= "data.addRow(['{$d_day}{$d_detent_time}',$q_totalsale1_notax_price]);\n";
