...
$sql = "select free_class from pos_zfree where pos_shopno = $pos_shopno and to_char(sale_date,'YYYY-MM') = '$selyymm' and status = '1' group by free_class order by free_class";
...
<form action="zfree_view.php" method="post" id="form1">
...
$sql = "select sale_date,free_code,max(free_code_name),sum(totalsale1_notax_price),sum(totalsale1_notax_score),sum(num_custom),sum(num_pair),sum(totalsale3_notax_price) from pos_zfree where pos_shopno = $pos_shopno and sale_date {$date_sql} and free_class = $free_class_id and status = '1' group by sale_date,free_class,free_code order by sale_date,free_class,free_code";
...
for($i=0;$i<$num;$i++){
	list($q_sale_date,$q_free_code,$free_code_name,$q_totalsale1_notax_price,$q_totalsale1_notax_score,$q_num_custom,$q_num_pair,$q_totalsale3_notax_price) = pg_fetch_array($result,$i);
	$free_code_name = trim($free_code_name);
...
$ybase->ST_PRI .= <<<HTML
<td align="center">$free_code_name</td>
...
HTML;
$graphtxt .= "data.addRow(['{$d_day}$free_code_name',$q_totalsale1_notax_price]);\n";
