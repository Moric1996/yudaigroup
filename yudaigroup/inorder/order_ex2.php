...
$product_other = pg_escape_string($product_other);
$photo_place = pg_escape_string($photo_place);
...
foreach($tool_name as $key => $val){
	...
	$in_delivery = pg_escape_string($in_delivery);
	$in_papersize = trim($papersize[$key]);
	if(!$in_papersize){
		$in_papersize = "null";
	}
	$in_laminate = trim($laminate[$key]);
	$in_toolbiko = trim($toolbiko[$key]);
	$in_toolbiko = pg_escape_string($in_toolbiko);
	...
	$sql = "insert into order_tool (tool_id,order_id,num,tool_name,quantity,delivery,papersize,toolbiko,add_date,status,laminate) values ($new_tool_id,$new_order_id,$key,'$val',$in_quan,'$in_delivery',$in_papersize,'$in_toolbiko','now','1','$in_laminate')";
	$result = $ybase->sql($conn,$sql);
}
...
foreach($photo_name as $key => $val){
	...
	$in_foodst = pg_escape_string($in_foodst);
	$in_photo_biko = trim($photo_biko[$key]);
	$in_photo_biko = pg_escape_string($in_photo_biko);
