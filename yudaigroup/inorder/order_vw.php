...
$sql = "select tool_id,num,tool_name,quantity,delivery,papersize,toolbiko,tool_emp_id from order_tool where order_id = $t_order_id and status > '0' order by num";
$result = $ybase->sql($conn,$sql);
$tool_num = pg_num_rows($result);
for($i=0;$i<$tool_num;$i++){
	list($q_tool_id,$q_num,$q_tool_name,$q_quantity,$q_delivery,$q_papersize,$q_toolbiko,$q_tool_emp_id) = pg_fetch_array($result,$i);
