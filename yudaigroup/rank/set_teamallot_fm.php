...
if(!$num3){
	...
	$sql = "insert into telecom_goal_group (shop_id,item_id,month,group_id,goal_num,add_date,status) select shop_id,item_id,$target_month,group_id,goal_num,'now',status from telecom_goal_group where month = $last_month and shop_id = $target_shop_id and status = '1'";
...
$ybase->title = "Y☆Rank-月間目標・チーム配分設定";
...
$(function($) {
	...
	$("[id^=reset_allot]").click(function(){
		if(!confirm('現在の配分率で各グループの配分を再設定し直します。よろしいですか？')){
			return false;
		}else{
			location.href = './set_teamallot_fullauto.php?target_month=$target_month&target_shop_id=$target_shop_id';
	    }
	});
});
</script>
...
<div style="text-align:left;"><a class="btn btn-info btn-sm" href="#!" role="button" id="reset_allot">現在の分配率で配分を再設定</a></div>
