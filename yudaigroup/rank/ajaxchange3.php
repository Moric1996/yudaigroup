...
switch ($colname){
	case "group_name":
		$tablename = "telecom_group";
		$ino_colname = "group_id";
		$value = "'$var_val'";
		break;
	case "leader_employee_id":
		if($var_val){
		$sql = "select employee_id from telecom_group_const where month = $target_month and shop_id = $target_shop_id and group_id = $ino and status = '1' and group_const_id = $var_val";
	...
	case "allot":
		...
		break;
	case "employee_id":
		$sql = "select employee_id from telecom_group_const where month = $target_month and shop_id = $target_shop_id and group_const_id = $ino and status = '1'";
		$result = $ybase->sql($conn,$sql);
		$num = pg_num_rows($result);
		if($num == 1){
			...
			if($var_val && $target_group_id && $pre_employee_id){
				$sql = "update telecom_group set leader_employee_id = $var_val where month = $target_month and shop_id = $target_shop_id and leader_employee_id = $pre_employee_id and group_id = $target_group_id and status = '1'";
				$result = $ybase->sql($conn,$sql);
			}
		}
		$tablename = "telecom_group_const";
