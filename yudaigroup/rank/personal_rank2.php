...
if($all_ca == 1){
	...
	foreach($rank_section_name as $key => $val){
		$car_link .= "<a href=\"personal_rank2.php?t_month=$t_month&target_day=$target_day&t_shop_id=$key\" class=\"btn btn-sm btn-outline-warning\">$val</a> ";
	}

}else{
	$addsql = "month = $t_month and shop_id = $t_shop_id and status = '1'";
	$carria_name = $rank_section_name[$t_shop_id];
	$car_link = "<a href=\"personal_rank2.php?$param&target_day=$target_day&all_ca=1\" class=\"btn btn-sm btn-outline-warning\">全店</a> ";
	foreach($rank_section_name as $key => $val){
...
/////////////////////////グループリスト
$sql = "select group_id,group_name,allot,shop_id from telecom_group where {$addsql} and status = '1' order by group_id";
...
/////////////////////////グループ構成員リスト
$sql = "select group_const_id,group_id,employee_id,shop_id,short_name from telecom_group_const where {$addsql} and status = '1' order by group_id,group_const_id";
$result = $ybase->sql($conn,$sql);
...
for($i=0;$i<$const_num;$i++){
	list($q_group_const_id,$q_group_id,$q_employee_id,$q_shop_id,$q_short_name) = pg_fetch_array($result,$i);
	$group_emp_lt[$q_employee_id] = $q_group_id;
	$emp_sname_lt[$q_employee_id] = $q_short_name;
...
$ybase->title = "Y☆Rank-個別実績";

$ybase->HTMLheader();
$ybase->ST_PRI .= $ybase->header_pri("個別実績");
...
<p></p>
<h5 style="text-align:center;">【{$rank_section_name[$t_shop_id]} {$YEAR}年{$MONTH}月{$target_day}日】個別実績</h5>

<p></p>
<a href="personal_rank2.php?$param&target_day=$bf" class="btn btn-sm btn-outline-secondary{$bf_disable}">前日</a>
<a href="personal_rank2.php?$param&target_day=$nx" class="btn btn-sm btn-outline-secondary{$nx_disable}">翌日</a>
&nbsp;<a href="group_rank2.php?$param&target_day=$target_day" class="btn btn-sm btn-outline-info">チーム</a>
&nbsp;$car_link
...
for($i=0;$i<$num3;$i++){
	...
	$group_action_lt[$q_employee_id] += $q_action_num * $item_score_lt[$q_itemid];
}
/////////////////////////////////////////////
...
$gcolor = $group_color_lt[$groupid];
$shopid = $group_shop_lt[$groupid];
if($all_ca){
...
<td bgcolor="{$nam_color}"><!--{$rank_section_name[$shopid]} {$group_name_lt[$groupid]}--> <b>{$sec_employee_list[$key]}</b></td>
