...
$sql = "select group_id,count(*) from telecom_group_const where {$addsql} and status = '1' group by group_id order by group_id";
...
if(!$num){
	...
	$sql = "update telecom_group set status = '0' where {$addsql} and status = '1'";
	$result = $ybase->sql($conn,$sql);

	$sql = "insert into telecom_group (group_id,group_name,shop_id,month,leader_employee_id,allot,add_date,status) select group_id,group_name,shop_id,$target_month,leader_employee_id,allot,'now',status from telecom_group where month = $last_month and shop_id = $target_shop_id and status = '1' order by group_id";
	$result = $ybase->sql($conn,$sql);

	$sql = "insert into telecom_group_const (group_const_id,shop_id,month,group_id,employee_id,add_date,status,short_name) select group_const_id,shop_id,$target_month,group_id,employee_id,'now',status,short_name from telecom_group_const where month = $last_month and shop_id = $target_shop_id and status = '1' order by group_id,group_const_id";
	$result = $ybase->sql($conn,$sql);
...
$sql = "select group_id,group_name,leader_employee_id,allot from telecom_group where {$addsql} and status = '1' order by group_id";

$result = $ybase->sql($conn,$sql);
$num = pg_num_rows($result);
if(!$num){
	$sql = "insert into telecom_group (group_id,group_name,shop_id,month,add_date,status) values (nextval('telecom_group_id_seq'),'',$target_shop_id,$target_month,'now','1')";
	$result = $ybase->sql($conn,$sql);
	$sql = "select group_id,group_name,leader_employee_id,allot from telecom_group where {$addsql} and status = '1' order by group_id";
	$result = $ybase->sql($conn,$sql);
	$num = pg_num_rows($result);
...
$sql = "select group_const_id,employee_id,short_name from telecom_group_const where {$addsql} and group_id = $q_group_id and status = '1' order by group_const_id";

$result2 = $ybase->sql($conn,$sql);
$num2 = pg_num_rows($result2);
if(!$num2){
	$sql = "insert into telecom_group_const (group_const_id,shop_id,month,group_id,add_date,status) values (nextval('telecom_group_const_id_seq'),$target_shop_id,$target_month,$q_group_id,'now','1')";
	$result = $ybase->sql($conn,$sql);
	$sql = "select group_const_id,employee_id,short_name from telecom_group_const where {$addsql} and group_id = $q_group_id and status = '1' order by group_const_id";
