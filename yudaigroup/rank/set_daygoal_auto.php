...
/////////////////////////目標確認
$sql = "select item_id,sum(goal_num) from telecom_goal_group where {$addsql} and status = '1' group by item_id order by item_id";
...
///////////////////////////////////////////////////////////
$sql = "update telecom_goal_day set status = '0' where {$addsql} and status = '1'";
$result = $ybase->sql($conn,$sql);


$sql = "select item_id,score from telecom_item where {$addsql} and status = '1' order by bigitem_id,order_num";
$result = $ybase->sql($conn,$sql);
$num = pg_num_rows($result);
if(!$num){
...
for($i=0;$i<$num;$i++){
	...
	for($k=1;$k<=$maxday;$k++){
		...
		$gokei += $in_goal;
		$sql0 = "insert into telecom_goal_day (shop_id,item_id,month,day,dgoal_num,add_date,status) values ($target_shop_id,$q_item_id,$target_month,$k,$in_goal,'now','1')";
