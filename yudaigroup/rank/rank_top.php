...
$rank = new rank();
$ybase->session_get();
$rank_section_list['0'] = 0;
$rank_section_name['0'] = "Y☆Shop";
$ybase->section_list['0'] = "Y☆Shop";

$rank_section_list['1'] = 0;
$rank_section_name['1'] = "CS☆Board";
$ybase->section_list['1'] = "CS☆Board";

ksort($rank_section_list);
ksort($rank_section_name);
ksort($ybase->section_list);
...
foreach($rank_section_list as $key => $val){
	if($t_shop_id && ($val != $t_shop_id) && ($key > 1)){
		continue;
	}
...
if($q_com_date[$key]){
	$dis_c_date = $q_com_date[$key];
}else{
	$dis_c_date = "　　　　　　　　";
}
if($sec_employee_list[$emp_id]){
	$dis_c_employee = $sec_employee_list[$emp_id];
}else{
	$dis_c_employee = "　　　　　";
}
$ybase->ST_PRI .= <<<HTML
...
</td><td align="center"><span style="font-size:80%;">{$ybase->section_list[$key]}コメント</td></span><td align="right"><span style="font-size:60%;">{$dis_c_date}　{$dis_c_employee}</span>　<a href="#!" class="btn btn-sm btn-outline-info" id="slidebutton$key" shopid="$key">{$slidebutton[$key]}</a></td>
</tr>
</table>
...
<br>
<div class="row">
<div class="col-sm-6 offset-sm-3"><a class="btn btn-outline-info btn-block{$linkable}" href="./commentall_reg.php?$param" role="button">Y☆Shop投稿用</a></div>
</div>
<br>
<div class="row">
<div class="col-sm-6 offset-sm-3"><a class="btn btn-outline-info btn-block{$linkable}" href="./commentall_reg.php?$param&sec_id=1" role="button">CS☆Board投稿用</a></div>
</div>
<br>
<div class="row">
