...
//////////////////////////////////////////項目数
$sql = "select count(item_id) from telecom_item where {$addsql} and status = '1'";
...
foreach($group_name_lt as $key => $val){
	$sql = "select group_const_id,employee_id from telecom_group_const where {$addsql} and group_id = $key and status = '1' order by group_const_id";
	$result = $ybase->sql($conn,$sql);
	$num = pg_num_rows($result);
	$finish_cnt = 0;
	for($i=0;$i<$num;$i++){
		...
		$sql_p = "select item_id from telecom_goal where {$addsql} and employee_id = $q_employee_id and status = '1' order by item_id";
