...
$sql = "select item_id,u_name from telecom_unitname where {$addsql} and status = '1' order by item_id";
...
if(!$num){
	...
	$sql = "update telecom_bigitem set status = '0' where {$addsql} and status = '1'";
	$result = $ybase->sql($conn,$sql);

	$sql = "insert into telecom_bigitem (bigitem_id,bigitem_name,shop_id,month,order_num,add_date,status) select bigitem_id,bigitem_name,shop_id,$target_month,order_num,'now',status from telecom_bigitem where month = $last_month and shop_id = $target_shop_id and status = '1' order by bigitem_id";
	$result = $ybase->sql($conn,$sql);

	$sql = "insert into telecom_item (item_id,item_name,shop_id,month,bigitem_id,score,order_num,add_date,status) select item_id,item_name,shop_id,$target_month,bigitem_id,score,order_num,'now',status from telecom_item where month = $last_month and shop_id = $target_shop_id and status = '1' order by item_id";
	$result = $ybase->sql($conn,$sql);

	$sql = "insert into telecom_unitname (shop_id,month,item_id,u_name,add_date,status) select shop_id,$target_month,item_id,u_name,'now',status from telecom_unitname where month = $last_month and shop_id = $target_shop_id and status = '1' order by item_id";
	$result = $ybase->sql($conn,$sql);
	$sql = "select item_id,u_name from telecom_unitname where {$addsql} and status = '1' order by item_id";
	$result = $ybase->sql($conn,$sql);
	$num = pg_num_rows($result);
...
$num = pg_num_rows($result);
if(!$num){
	$sql = "insert into telecom_bigitem (bigitem_id,bigitem_name,shop_id,month,order_num,add_date,status) values (nextval('telecom_bigitem_id_seq'),'',$target_shop_id,$target_month,1,'now','1')";
	$result = $ybase->sql($conn,$sql);
	$sql = "select bigitem_id,bigitem_name from telecom_bigitem where {$addsql} and status = '1' order by order_num";
	$result = $ybase->sql($conn,$sql);
	$num = pg_num_rows($result);
...
$sql = "select item_id,action_num from telecom_action where {$addsql} and status = '1' order by item_id";
...
if(!$num2){
	$sql = "insert into telecom_item (item_id,item_name,shop_id,month,bigitem_id,score,order_num,add_date,status) values (nextval('telecom_item_id_seq'),'',$target_shop_id,$target_month,$q_bigitem_id,0,1,'now','1')";
	$result2 = $ybase->sql($conn,$sql);
	$sql = "select item_id,item_name,score,noinput from telecom_item where {$addsql} and bigitem_id = $q_bigitem_id and status = '1' order by order_num";
	$result2 = $ybase->sql($conn,$sql);
	$num2 = pg_num_rows($result2);
...
<a href="./set_bigitem_updn.php?updn=up&bigitem_id=$q_bigitem_id&$param">▲</a>|
<a href="./set_bigitem_updn.php?updn=dn&bigitem_id=$q_bigitem_id&$param">▼</a>
