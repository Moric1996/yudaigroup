...
/////////////////////////
$sql = "select employee_id from telecom_group_const where {$addsql} and status = '1'";
...
$sql = "select employee_id from telecom_action where {$addsql} and status = '1'";
$result = $ybase->sql($conn,$sql);
$num = pg_num_rows($result);
if($num){
	...
	foreach($diff_arr as $key => $val){
		$sql = "update telecom_action set status = '0' where {$addsql} and employee_id = $val and status = '1'";
		$result = $ybase->sql($conn,$sql);
	}
}

$sql = "select employee_id from telecom_goal where {$addsql} and status = '1'";
$result = $ybase->sql($conn,$sql);
$num = pg_num_rows($result);
if($num){
	...
	foreach($diff_arr as $key => $val){
		$sql = "update telecom_goal set status = '0' where {$addsql} and employee_id = $val and status = '1'";
		$result = $ybase->sql($conn,$sql);
	}
}
