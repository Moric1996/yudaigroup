...
//////////////////////////////////////////項目確認
$sql = "select item_id from telecom_item where {$addsql} and status = '1' and item_name <> '' and score is not null order by item_id";
...
//////////////////////////////////////////グループ確認
$sql = "select group_id,group_name from telecom_group where {$addsql} and status = '1' and group_name <> '' and leader_employee_id is not null order by group_id";
$result = $ybase->sql($conn,$sql);
...
$sql = "select group_id,count(*) from telecom_group_const where {$addsql} and status = '1' and employee_id is not null group by group_id order by group_id";
$result = $ybase->sql($conn,$sql);
...
$sql = "select group_id from telecom_group_const where {$addsql} and status = '1' and employee_id is null order by group_id";
$result = $ybase->sql($conn,$sql);
$num = pg_num_rows($result);
if($num){
	$group_flag=0;
...
if($group_goal_num > $check_group_goal){
	$sql = "update telecom_goal_group set status = '0' where {$addsql} and status = '1' and item_id not in (select item_id from telecom_item where {$addsql} and status = '1')";
	$result = $ybase->sql($conn,$sql);
	$sql = "update telecom_goal_group set status = '0' where {$addsql} and status = '1' and group_id not in (select group_id from telecom_group where {$addsql} and status = '1')";
	$result = $ybase->sql($conn,$sql);
	$sql = "select item_id,group_id,goal_num from telecom_goal_group where {$addsql} and status = '1' order by item_id,group_id";
	$result = $ybase->sql($conn,$sql);
	$group_goal_num = pg_num_rows($result);
...
//////////////////////////////////////////チーム個別配分確認
$sql = "select employee_id from telecom_group_const where {$addsql} and status = '1' order by group_const_id";
$result = $ybase->sql($conn,$sql);
$emp_num = pg_num_rows($result);
$check_emp_goal = $item_num * $emp_num;

$sql = "select item_id,employee_id,goal_num from telecom_goal where {$addsql} and status = '1' order by item_id,employee_id";
$result = $ybase->sql($conn,$sql);
$emp_goal_num = pg_num_rows($result);
////////////////////不整合データの削除
if($emp_goal_num > $check_emp_goal){
	$sql = "update telecom_goal set status = '0' where {$addsql} and status = '1' and item_id not in (select item_id from telecom_item where {$addsql} and status = '1')";
	$result = $ybase->sql($conn,$sql);
	$sql = "update telecom_goal set status = '0' where {$addsql} and status = '1' and employee_id not in (select employee_id from telecom_group_const where {$addsql} and status = '1')";
	$result = $ybase->sql($conn,$sql);
	$sql = "select item_id,employee_id,goal_num from telecom_goal where {$addsql} and status = '1' order by item_id,employee_id";
	$result = $ybase->sql($conn,$sql);
	$emp_goal_num = pg_num_rows($result);
...
$sql = "select item_id,day,dgoal_num from telecom_goal_day where {$addsql} and status = '1' order by item_id,day";
$result = $ybase->sql($conn,$sql);
$day_num = pg_num_rows($result);
$check_day_goal = $item_num * $maxday;
////////////////////不整合データの削除
if($day_num > $check_day_goal){
	$sql = "update telecom_goal_day set status = '0' where {$addsql} and status = '1' and item_id not in (select item_id from telecom_item where {$addsql} and status = '1')";
	$result = $ybase->sql($conn,$sql);
	$sql = "select item_id,day,dgoal_num from telecom_goal_day where {$addsql} and status = '1' order by item_id,day";
	$result = $ybase->sql($conn,$sql);
	$day_num = pg_num_rows($result);
...
<div class="container">
<p></p>
<div style="text-align:right;"><a class="btn btn-secondary btn-sm" href="./rank_top.php?$param0" role="button">Y☆RankTOPに戻る</a></div>
