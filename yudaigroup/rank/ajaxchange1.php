...
$sql = "select action_num from telecom_action where month = $target_month and shop_id = $target_shop_id and day = $target_day and employee_id = $target_employee_id and item_id = $target_item_id and status = '1'";
$result = $ybase->sql($conn,$sql);
$num = pg_num_rows($result);
if(!$num){
	$sql = "insert into telecom_action (shop_id,item_id,month,day,employee_id,action_num,add_date,status) values ($target_shop_id,$target_item_id,$target_month,$target_day,$target_employee_id,$value,'now','1')";
}else{
	$sql = "update telecom_action set $colname = $value where month = $target_month and shop_id = $target_shop_id and day = $target_day and employee_id = $target_employee_id and item_id = $target_item_id and status = '1'";
}
