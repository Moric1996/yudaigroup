...
$tcpdf->SetFont("kozminproregular", "", 5);
...
$ybase->ST_PRI .= <<<HTML
<h3 style="text-align:center;">【{$rank_section_name[$t_shop_id]} {$YEAR}年{$MONTH}月】日別実績(全体)</h5>
...
if(strlen($q_item_name) < 45){
	$rowheight = " height=\"9\"";
}else{
	$rowheight = "";
//	$q_item_name = mb_substr($q_item_name,0,10);
	if(($i == 2) && ($ii == 11)){
//		$q_item_name = str_replace("d","<br>d",$q_item_name);
//$aaa = "※ショップ・・・→ショップアプリdカードスタンプカード";
//	$str = "*".$addrowscnt;
//	$q_item_name = '';
