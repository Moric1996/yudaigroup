...
//目標
if($target_group_id == 3059999){//SBの場合の大仁A大仁Bを同時に出す処理
	$sql = "select item_id,sum(goal_num) from telecom_goal_group where {$addsql} and group_id in ($sb_group_oohito1,$sb_group_oohito2) group by item_id order by item_id";
}else{
	$sql = "select item_id,sum(goal_num) from telecom_goal_group where {$addsql} and group_id = $target_group_id group by item_id order by item_id";
}
$result3 = $ybase->sql($conn,$sql);
$num3 = pg_num_rows($result3);
for($i=0;$i<$num3;$i++){
...
$sql = "select item_id,day,sum(action_num) from telecom_action where {$addsql} and employee_id in ({$emp_lists}) group by item_id,day order by item_id,day";
$result3 = $ybase->sql($conn,$sql);
$num3 = pg_num_rows($result3);
for($i=0;$i<$num3;$i++){
