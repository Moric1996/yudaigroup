...
$(function(){
	$("[id^=plus]").click(function(){
		...
		if((itemid == 145)||(itemid == 146)||(itemid == 147)||(itemid == 149)||(itemid == 150)||(itemid == 152)||(itemid == 546)){
			...
			var nx_val{$smtab} = +nx_val143 - action_val151;
			if(nx_val{$smtab} < 0){
				nx_val{$smtab} = 0;
			}
		...
		if((itemid == 145)||(itemid == 146)||(itemid == 595)||(itemid == 546)){
		...
		if(itemid == 151){
			var now_score{$smtab} = +action_val{$smtab} * {$item_score[$smtab]};
			var nexval{$smtab} = +action_val143 - action_val151 - 1;
			if(nexval{$smtab} < 0){
				nexval{$smtab} = 0;
			}
...
$(function(){
	$("[id^=p10lus]").click(function(){
		...
		todayn = +todayn + +my_score;
		if((itemid == 145)||(itemid == 146)||(itemid == 147)||(itemid == 149)||(itemid == 150)||(itemid == 152)||(itemid == 546)){
			var nx_val143 = +action_val143 + 10;
			action_val143 = nx_val143;
			$('#action143').val(nx_val143);
			$('#action143').ajaxchange_js();
			$('#dis_action143').text(nx_val143);
		...
		if((itemid == 145)||(itemid == 146)||(itemid == 595)||(itemid == 546)){
			var nx_val144 = +action_val144 + 10;
			$('#action144').val(nx_val144);
			$('#action144').ajaxchange_js();
			$('#dis_action144').text(nx_val144);
			todayn = +todayn + +{$item_score[144]};
		...
		if(itemid == 151){
			var now_score{$smtab} = +action_val{$smtab} * {$item_score[$smtab]};
			var nexval{$smtab} = +action_val143 - action_val151 - 10;
			if(nexval{$smtab} < 0){
				nexval{$smtab} = 0;
			}
			var nex_score{$smtab} = +nexval{$smtab} * {$item_score[$smtab]};
...
<form action="./person_in.php" method="post" id="Form1">
...
<tr><td>
<form action="person_in.php" method="get" id="dayselectfrom">
<input type="hidden" name="t_month" value="$t_month">
<input type="hidden" name="t_shop_id" value="$t_shop_id">
...
<td>
<nobr>
<a href="person_in.php?$param&target_day=$bf&target_employee_id=$target_employee_id" class="btn btn-sm btn-outline-secondary{$bf_disable}">前日</a>
<a href="person_in.php?$param&target_day=$nx&target_employee_id=$target_employee_id" class="btn btn-sm btn-outline-secondary{$nx_disable}">翌日</a>
</nobr>
</td><td align="right">
...
<br>
<br>
<button type="button" class="btn btn-info rounded-circle p-0" style="width:2rem;height:2rem;" id="m10inus$q_item_id" item_id="$q_item_id" score="$q_score"{$indisabled}>-10</button>　
<button type="button" class="btn btn-info rounded-circle p-0" style="width:2rem;height:2rem;" id="p10lus$q_item_id" item_id="$q_item_id" score="$q_score"{$indisabled}>+10</button>
HTML;
