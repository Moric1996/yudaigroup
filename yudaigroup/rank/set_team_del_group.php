...
$sql = "select group_const_id,employee_id from telecom_group_const where {$addsql} and group_id = $t_group_id and status = '1'";
...
for($i=0;$i<$num0;$i++){
	...
	if($q_employee_id){
	$sql = "select employee_id from telecom_group_const where {$addsql} and group_const_id <> $q_group_const_id and status = '1' and employee_id = $q_employee_id";
...
$sql = "update telecom_group_const set status = '0' where {$addsql} and group_id = $t_group_id and status = '1'";
$result = $ybase->sql($conn,$sql);

$sql = "update telecom_goal_group set status = '0' where {$addsql} and group_id = $t_group_id and status = '1'";
$result = $ybase->sql($conn,$sql);

$sql = "update telecom_group set status = '0' where {$addsql} and group_id = $t_group_id and status = '1'";
$result = $ybase->sql($conn,$sql);
