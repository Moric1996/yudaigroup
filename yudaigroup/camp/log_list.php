...
$ybase->title = "キャンペーン管理-シリアルコード入力エラーログ一覧";
...
<div class="container">
  <ul class="nav nav-tabs nav-fill" id="myTab" style="font-size:70%;">
    ...
    <li class="nav-item">
      <a href="./log_list.php" class="nav-link active">シリアルコード入力エラーログ</a>
    </li>
    <li class="nav-item">
...
<form action="log_list.php" method="post" id="form1">
...
$sql = "select log_id,code_id,shop_id,user_id,code,ip,cnt,to_char(add_date,'YYYY/MM/DD HH24:MI:SS'),status from stamp_log where camp_id = $camp_id and status = '1' order by log_id desc";
...
$mun0 = 0;
for($i=$st;$i<$end;$i++){
	list($q_log_id,$q_code_id,$q_shop_id,$q_user_id,$q_code,$q_ip,$q_cnt,$q_add_date,$q_status) = pg_fetch_array($result,$i);
...
{$page}／{$allpage}
<a href="log_list.php?$param&page=$bf" class="btn btn-sm btn-outline-secondary{$addbfclass}">＜</a>
<a href="log_list.php?$param&page=$nx" class="btn btn-sm btn-outline-secondary{$addnxclass}">＞</a>
</span>
</div>
</div>
