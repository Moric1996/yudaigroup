...
header("Location: https://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/view2.php?{$param}");
exit;
}
...
$('#filelist').tree({
	onClick: function(node){
		if(node.ext == "pdf"){
			var jurl = "../inc/pdfjs/web/viewer.html?file=/yudaigroup/documanage/viewpdf2.php%3Ffile_id%3D" + node.id;
		}
		if(node.ext == ""){
			var jurl = "../inc/pdfjs/web/viewer.html?file=/yudaigroup/documanage/viewpdf2.php%3Ffile_id%3D" + node.id;
		}
		if((node.ext == "mpeg")||(node.ext == "mp4")||(node.ext == "mov")||(node.ext == "mpg")||(node.ext == "avi")||(node.ext == "webm")){
			var jurl = "./movieviewer.php?company_id=$company_id&file_id=" + node.id;
		}
		...
		if((node.ext == "doc")||(node.ext == "xls")||(node.ext == "ppt")||(node.ext == "docx")||(node.ext == "xlsx")||(node.ext == "pptx")){
			var jurl = "./msviewer.php?company_id=$company_id&file_id=" + node.id;
		}
...
<iframe src="../inc/pdfjs/web/viewer.html?file=" width="100%" height="100%" id="pdfiframe"></iframe>
    </div>
    </div>
