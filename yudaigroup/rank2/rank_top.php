...
foreach($rank_section_list as $key => $val){
	...
	if(($comm_id == $key) && $comm_date){
		$sql = "select comment_id from telecom2_comment where shop_id = $key and status = '1' and to_char(add_date,'YYYY-MM-DD') <= '$comm_date' order by comment_id desc";
		...
		$num = pg_num_rows($result);
			if(!$num){
				...
				$sql = "select comment_id from telecom2_comment where shop_id = $key and status = '1' order by comment_id desc";
				$result = $ybase->sql($conn,$sql);
				$num = pg_num_rows($result);
				if(!$num){
					$comm_no[$key] = 0;
...
$sql = "select comment_id,employee_id,comment,to_char(add_date,'YYYY/MM/DD HH24:MI') from telecom2_comment where shop_id = $key and status = '1' order by comment_id desc";
