...
/////////////////////////
$sql = "select employee_id from telecom2_group_const where {$addsql} and group_const_id = $t_group_const_id and status = '1'";
...
if($num){
	...
	if($q_employee_id){
	$sql = "select employee_id from telecom2_group_const where {$addsql} and group_const_id <> $t_group_const_id and status = '1' and employee_id = $q_employee_id";
	$result = $ybase->sql($conn,$sql);
	...
	$sql = "update telecom2_group_const set status = '0' where {$addsql} and group_const_id = $t_group_const_id and status = '1'";
	$result = $ybase->sql($conn,$sql);
}
