...
$sql = "select item_id,action_num from telecom2_action where {$addsql} and day = $target_day and employee_id = $target_employee_id order by item_id";
...
$sql = "select bigitem_id,item_id,item_name,score from telecom2_item where {$addsql} and status = '1' order by order_num";
$result3 = $ybase->sql($conn,$sql);
$num3 = pg_num_rows($result3);
$today_act_num = 0;
for($i=0;$i<$num3;$i++){
...
$all_goal_score = 0;
//個人目標
$sql = "select item_id,goal_num from telecom2_goal where {$addsql} and employee_id = $target_employee_id order by item_id";
$result3 = $ybase->sql($conn,$sql);
$num3 = pg_num_rows($result3);
for($i=0;$i<$num3;$i++){
...
//当日実績個人
$all_total=0;
$sql = "select item_id,day,action_num from telecom2_action where {$addsql} and employee_id = $target_employee_id and day <= $target_day order by item_id,day";
$result3 = $ybase->sql($conn,$sql);
$num3 = pg_num_rows($result3);
for($i=0;$i<$num3;$i++){
...
<td rowspan="{$bigitem_cnt[$q_bigitem_id]}" style="vertical-align: middle;width:30px;color:{$bigcharcolor[$i]};" bgcolor="{$bcolor[$i]}">
$str
</td>
HTML;
}else{
