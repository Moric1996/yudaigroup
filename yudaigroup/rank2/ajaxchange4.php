...
$add_sql = " month = $target_month and shop_id = $target_shop_id and item_id = $target_item_id and group_id = $target_group_id and status = '1'";

switch ($colname){
	case "goal_num":
		$tablename = "telecom2_goal_group";
		...
		if(!$num){
			$sql = "insert into $tablename (shop_id,item_id,month,group_id,goal_num,add_date,status) values ($target_shop_id,$target_item_id,$target_month,$target_group_id,$value,'now','1')";
			$result = $ybase->sql($conn,$sql);
		}
		break;
}
