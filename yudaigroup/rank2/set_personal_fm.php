...
//////////////////////////////////////////グループ名取得
$sql = "select group_name from telecom2_group where {$addsql} and group_id = $target_group_id and status = '1'";
$result = $ybase->sql($conn,$sql);
$num = pg_num_rows($result);
if(!$num){
...
//////////////////////////////////////////グループ目標取得
$sql = "select item_id,goal_num from telecom2_goal_group where {$addsql} and group_id = $target_group_id and status = '1' order by item_id";
$result = $ybase->sql($conn,$sql);
$num = pg_num_rows($result);
if(!$num){
...
//////////////////////////////////////////itemscore取得
$score_list = "";
$sql = "select item_id,score from telecom2_item where {$addsql} and status = '1' order by order_num";
$result = $ybase->sql($conn,$sql);
$num = pg_num_rows($result);
if(!$num){
...
/////////////////////////グループ構成員リスト＆目標データ確認
$sql = "select group_const_id,employee_id from telecom2_group_const where {$addsql} and group_id = $target_group_id and status = '1' order by group_id,group_const_id";
$result = $ybase->sql($conn,$sql);
...
for($i=0;$i<$const_num;$i++){
	...
	$sql3 = "select item_id,goal_num from telecom2_goal where {$addsql} and employee_id = $q_employee_id and status = '1' order by item_id";
	...
	if(!$num3){
		...
		$sql4 = "insert into telecom2_goal (shop_id,item_id,month,employee_id,goal_num,add_date,status) select shop_id,item_id,$target_month,employee_id,goal_num,'now',status from telecom2_goal where month = $last_month and shop_id = $target_shop_id and employee_id = $q_employee_id and status = '1'";
