...
$item_name_lt = array();
/////////////////////////グループ構成員リスト
$sql = "select group_const_id,employee_id from telecom2_group_const where {$addsql} and group_id = $target_group_id and status = '1' order by group_const_id";
...
///////////////////////////////////////項目
$sql = "select item_id,item_name from telecom2_item where {$addsql} and status = '1' order by item_id";
...
////////////////////////////
$sql = "select item_id,employee_id,goal_num from telecom2_goal where {$addsql} and employee_id in ($emplist) and status = '1' order by item_id,employee_id";
$result = $ybase->sql($conn,$sql);
...
for($i=0;$i<$goal_num;$i++){
	...
	if($q_goal_num == ""){
		$sql2 = "update telecom2_goal set goal_num = 0 where {$addsql} and status = '1' and item_id = $q_item_id and employee_id = $q_employee_id";
...
foreach($group_emp_lt as $gkey => $gval){
	foreach($item_name_lt as $ikey => $ival){
		if(!preg_match("/^[0-9]+$/",$goal_num_lt[$ikey][$gval])){
			$sql = "insert into telecom2_goal (shop_id,item_id,month,employee_id,goal_num,add_date,status) values ($target_shop_id,$ikey,$target_month,$gval,0,'now','1')";
			$result = $ybase->sql($conn,$sql);
		}
	}
}
