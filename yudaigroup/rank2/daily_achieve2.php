...
$sql = "select item_id,day,sum(action_num) from telecom2_action where {$addsql} group by item_id,day order by item_id,day";
$result3 = $ybase->sql($conn,$sql);
$num3 = pg_num_rows($result3);
for($i=0;$i<$num3;$i++){
...
$ybase->ST_PRI .= <<<HTML
<tr bgcolor="$hbgcolor">
<td rowspan="{$bigitem_cnt[$q_bigitem_id]}" style="vertical-align: middle;color:{$bigcharcolor[$i]};" bgcolor="{$bcolor[$i]}" width="12">
$str
</td>
HTML;
}else{
$ybase->ST_PRI .= <<<HTML
<tr bgcolor="$hbgcolor" valign="middle">
HTML;
}
...
$ybase->ST_PRI .= <<<HTML
<td align="right" width="19"{$rowheight}>{$actnum_arr[$q_item_id][$k]}</td>
HTML;
}
