...
///////////////////////////////////////項目
$sql = "select bigitem_id,order_num from telecom2_item where {$addsql} and item_id = $item_id";
...
////////////////////////////
if($updn == "up"){
$sql = "select item_id,order_num from telecom2_item where {$addsql} and bigitem_id = $q_bigitem_id and order_num < $q_order_num and status = '1' and item_id <> $item_id order by order_num desc";
}elseif($updn == "dn"){
$sql = "select item_id,order_num from telecom2_item where {$addsql} and bigitem_id = $q_bigitem_id and order_num > $q_order_num and status = '1' and item_id <> $item_id order by order_num";
}else{
...
$sql = "update telecom2_item set order_num = $q_order_num where {$addsql} and bigitem_id = $q_bigitem_id and item_id = $qcg_item_id";
$result = $ybase->sql($conn,$sql);

$sql = "update telecom2_item set order_num = $qcg_order_num where {$addsql} and bigitem_id = $q_bigitem_id and item_id = $item_id";
$result = $ybase->sql($conn,$sql);
