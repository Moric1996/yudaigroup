...
if(!preg_match("/^[0-9]+$/",$ino2)){
	$ybase->error("パラメーターエラー。ERROR_CODE:10826");
}
...
$add_sql = " month = $target_month and shop_id = $target_shop_id and item_id = $target_item_id and day = $ino2 and status = '1'";

switch ($colname){
	case "dgoal_num":
		$tablename = "telecom2_goal_day";
		...
		$sql = "select dgoal_num from $tablename where{$add_sql}";
		$result = $ybase->sql($conn,$sql);
		$num = pg_num_rows($result);
		if(!$num){
			$sql = "insert into $tablename (shop_id,item_id,month,day,dgoal_num,add_date,status) values ($target_shop_id,$target_item_id,$target_month,$ino2,$value,'now','1')";
			$result = $ybase->sql($conn,$sql);
		}
		break;
}
