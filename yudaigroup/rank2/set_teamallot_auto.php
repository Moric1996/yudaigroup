...
for($i=0;$i<$goal_num;$i++){
	...
	if($q_goal_num == ""){
		$sql2 = "update telecom2_goal_group set goal_num = 0 where {$addsql} and status = '1' and item_id = $q_item_id and group_id = $q_group_id";
...
foreach($group_name_lt as $gkey => $gval){
	foreach($item_name_lt as $ikey => $ival){
		if(!preg_match("/^[0-9]+$/",$goal_num_lt[$ikey][$gkey])){
			$sql = "insert into telecom2_goal_group (shop_id,item_id,month,group_id,goal_num,add_date,status) 
